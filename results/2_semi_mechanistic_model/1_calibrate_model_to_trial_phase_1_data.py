...
def define_log_posterior():
    ...
    measurements_df = pd.read_csv(
        directory + '/data/trial_phase_I.csv')

    # Define hierarchical log-posterior
    mechanistic_model,_ = define_hamberg_model(pk_only=True)
    mechanistic_model.set_outputs(['global.concentration_central_compartment'])
    ...
    problem.set_data(measurements_df, output_observable_dict={
        'global.concentration_central_compartment':
...
def run_inference(log_posterior):
    ...
    controller.set_transform(pints.ComposedTransformation(
        pints.IdentityTransformation(n_parameters=60 * 2 + 2),
    ...
    posterior_samples.sel(
        ...
        directory +
        '/posteriors/posterior_trial_phase_I.nc'
    )
