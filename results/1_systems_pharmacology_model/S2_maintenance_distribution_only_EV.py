...
def define_model():
    ...
    return mechanistic_model, parameters


def get_initial_dosing_regimen(covariates, offset, delta_t):
    """
...
def generate_data(mechanistic_model, parameters, nids):
    ...
    for idc in range(nids):
        ...
        cov = [0, 0, 0, 71, 0]
        regimen, dose_rates = get_initial_dosing_regimen(cov, warmup, delta_t)
...
if __name__ == '__main__':
    n = 100
    mm, p = define_model()
    d = generate_data(mm, p, n)

    # Save data to .csv
    directory = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    d.to_csv(directory + '/data/S2_maintenance_distribution_only_EV.csv')
