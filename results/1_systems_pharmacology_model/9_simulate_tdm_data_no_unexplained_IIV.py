...
def define_model():
    ...
    m = chi.PredictiveModel(model, chi.LogNormalErrorModel())
    m = chi.PopulationPredictiveModel(m, pop_model)
    parameters = np.array([
        parameters_df[parameters_df.Parameter == p].Value.values[0]
        for p in m.get_parameter_names()])

    # Set random IIV to effectively zero
    for idn, n in enumerate(m.get_parameter_names()):
        if ('std.' in n) or ('Std.' in n):
            parameters[idn] = 0.00001

    return model, pop_model, parameters
...
def generate_measurement(
        model, patient, pop_model, parameters, n, delays, vk_input, rng):
    """
    ...
    parameters = pop_model.sample(parameters, covariates=covariates)[0]
    ...
    inr = model.simulate(
        parameters=parameters[:-1], times=times, vk_input=vk_input[:n])[:, -1]

    # Add noise
    error_model = chi.LogNormalErrorModel()
    measurement = error_model.sample(
        parameters=parameters[-1:], model_output=inr, seed=rng)[0, 0]
...
if __name__ == '__main__':
    ...
    model, pop_model, parameters = define_model()
    ...
    for idp, patient in data.iterrows():
        ...
        measurements[idp] = generate_measurement(
            model, patient, pop_model, parameters, n, d, vk, rng)
