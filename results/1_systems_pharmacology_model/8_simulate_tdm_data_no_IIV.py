...
def generate_measurement(model, parameters, n, delays, vk_input, rng):
    """
    Generates TDM data for individual.
    """
    # Define noise level
    sigma_log = np.array([0.1])
...
if __name__ == '__main__':
    ...
    # Load patient data
    # NOTE: Not used here, just to count individuals.
    directory = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    data = pd.read_csv(directory + '/data/mipd_trial_cohort.csv')

    # Define model
    model, parameters_df = define_wajima_model(
