...
def define_model():
    ...
    for idn, n in enumerate(model.get_parameter_names()):
...
def define_demographics():
    ...
    covariates = np.empty((len(ids), 5))

    for idx, _id in enumerate(ids):
        temp = data[data.ID == _id]
        covariates[idx, [0, 1, 4]] = \
            temp[temp.Observable == 'VKORC1'].Value.values[0]
        covariates[idx, 2] = 0
        covariates[idx, 3] = 71
...
if __name__ == '__main__':
    ...
    c = define_demographics()
    d = generate_data(mm, em, pm, p, c)

    # Save data to .csv
    directory = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    d.to_csv(
        directory + '/data/S9_maintenance_distribution_no_unexplained_IIV.csv')
