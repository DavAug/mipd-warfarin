...
def get_mse_to_target_of_simulation_run(
    ...
    response = np.empty((len(times), 3, 5))
    response[:, 0, 0] = times
...
def plot_progress(performance, best_epoch):
    ...
    plt.savefig(
        directory + '/S5_dqn_training_random_iiv.pdf', bbox_inches='tight')
    plt.savefig(
        directory + '/S5_dqn_training_random_iiv.tif', bbox_inches='tight')
...
def plot_response(response, target):
    ...
    directory = os.path.dirname(os.path.abspath(__file__))
    plt.savefig(
        directory + '/S6_dqn_best_response_random_iiv.pdf',
        bbox_inches='tight')
    plt.savefig(
        directory + '/S6_dqn_best_response_random_iiv.tif',
        bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    data = generate_data(
        model, parameters, target=target, epsilon=0.05, n_episodes=1500)
    train, test = format_data(
        data, policy_net, target_net, target, rng, device)
    q_model = train_q_learning_algorithm(
        train, test, batch_size, n_epochs=30, gamma=0.5, device=device,
        model=model, parameters=parameters, target=target)

    # Save model
    directory = os.path.dirname(os.path.abspath(__file__))
    filename = '/models/dqn_hamberg_model_random_iiv.pickle'
