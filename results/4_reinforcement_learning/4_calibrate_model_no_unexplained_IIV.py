...
def simulate_buffer(covariates, policy):
    ...
    subprocess.Popen([
        ...
        '/2_semi_mechanistic_model/9_simulate_buffer_no_unexplained_IIV.py',
        '--filename',
        filename
    ]).wait()
...
def plot_progress(performance):
    ...
    directory = os.path.dirname(os.path.abspath(__file__))
    plt.savefig(
        directory + '/2_dqn_training_no_unexplained_IIV.pdf',
        bbox_inches='tight')
    plt.savefig(
        directory + '/2_dqn_training_no_unexplained_IIV.tif',
        bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    filename = '/models/dqn_model_performance_no_unexplained_IIV.npy'
