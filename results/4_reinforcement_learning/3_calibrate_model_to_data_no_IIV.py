...
def load_data(target):
    ...
    data = pd.read_csv(
        directory + '/data/S5_maintenance_distribution_no_IIV.csv')

    data = reshape_data(data, target)
...
def format_data(train, test, policy_net, target_net, target, device):
    ...
    # Normalise age
    mean = 71  # Age of all individuals in the training data
    train[:, -1] = (train[:, -1] - mean)
    test[:, -1] = (test[:, -1] - mean)

    # Make networks remember this scale
    policy_net.set_age_scale(mean, 1)
    target_net.set_age_scale(mean, 1)
...
def plot_progress(performance, best_epoch):
    ...
    plt.savefig(
        directory + '/2_dqn_training_no_IIV.pdf', bbox_inches='tight')
    plt.savefig(
        directory + '/2_dqn_training_no_IIV.tif', bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimiser, 'min', patience=10, min_lr=1e-6, factor=0.5)
