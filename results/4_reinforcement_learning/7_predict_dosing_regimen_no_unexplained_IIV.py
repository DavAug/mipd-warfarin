...
def generate_measurements(day, filename):
    ...
    subprocess.Popen([
        'python',
        directory +
        '/1_systems_pharmacology_model' +
        '/9_simulate_tdm_data_no_unexplained_IIV.py',
        '--number',
        str(n_obs),
        '--filename',
        filename
...
if __name__ == '__main__':
    ...
    for day in range(days):
        generate_measurements(day, filename)
        states = get_states(day, ids, covs, filename)
        doses = predict_doses(model, states, device)
        save_regimen(doses, ids, day, filename)
