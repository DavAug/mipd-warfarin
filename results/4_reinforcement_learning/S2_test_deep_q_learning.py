...
class DQN(nn.Module):
    ...
    def predict_dose(self, state):
        ...
        # Scale state
        state = (state - self._mean_inr) / self._std_inr
...
def train_q_learning_algorithm(
        ...
        model=None, parameters=None, target=None):
    """
...
def train_epoch(
    ...
    # Each epoch train only on have the dataset
    # NOTE: This is only so we converge in more than just 2 or 3 epochs
    n_data = n_data // 2
    data = data[:n_data]
...
def train_batch(batch, policy_net, target_net, optimiser, gamma, device):
    ...
    # (slowly updated)
    next_state_values = torch.zeros(state_batch.shape, device=device)
    with torch.no_grad():
...
def evaluate_test_set(data, policy_net, target_net, batch_size, gamma, device):
    ...
    # Tested only on have the dataset
    # NOTE: Avoids biased evaluation of performance
    n_data = n_data // 2
    data = data[:n_data]
...
def get_mse_to_target_of_simulation_run(
    ...
    for idt, time in enumerate(times):
        # Simulate model
        inr = model.sample(
            parameters=parameters, times=[time], return_df=False)[0, 0, 0]

        # Predict dose
        inr = torch.tensor(data=[[inr]], dtype=torch.float32, device=device)
        dose = policy_net.predict_dose(inr)
        dose = float(dose)
        doses.append(dose)
...
def plot_progress(performance, best_epoch):
    ...
    plt.savefig(
        directory + '/S3_dqn_training.pdf', bbox_inches='tight')
    plt.savefig(
        directory + '/S3_dqn_training.tif', bbox_inches='tight')
...
def plot_response(response, target):
    ...
    # Plot data
    axes[0].axhline(target, linestyle='dashed', color='blue', label='target')
    axes[0].plot(
        response[:, 0] / 24, response[:, 1], marker='x', color='black',
        label='response')
    axes[1].plot(
        response[:, 0] / 24, response[:, 2], marker='x', color='black')
    ...
    directory = os.path.dirname(os.path.abspath(__file__))
    plt.savefig(
        directory + '/S4_dqn_best_response.pdf', bbox_inches='tight')
    plt.savefig(
        directory + '/S4_dqn_best_response.tif', bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    data = generate_data(
        model, parameters, target=target, epsilon=0.05, n_episodes=1000)
    train, test = format_data(
        data, policy_net, target_net, target, rng, device)
    q_model = train_q_learning_algorithm(
        train, test, batch_size, n_epochs=10, gamma=0.5, device=device,
        model=model, parameters=parameters, target=target)

    # Save model
    directory = os.path.dirname(os.path.abspath(__file__))
    filename = '/models/dqn_hamberg_model_no_iiv.pickle'
